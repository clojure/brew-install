...
[source,shell]
----
# Set CLJ_SCRIPTS to any directory where you want to install the scripts
...
chmod +x clojure clj
perl -pi.bak -e "s,PREFIX,$CLJ_SCRIPTS,g" clojure
export PATH=$PATH:$CLJ_SCRIPTS
----
...
The `clojure` and `clj` scripts ultimately construct and invokes a command-line of the form:
...
** Contents:
*** `clojure` - main script
*** `clj` - wrapper script for interactive repl use (uses `rlwrap`)
*** `deps.edn` - initial user deps.edn file to be copied to the config directory
*** `clojure-scripts-X.Y.Z.jar` - uberjar invoked by `clojure` to construct classpaths
* config directory
