...
The following process can be used to manually install the `clj` script on a *nix system. Note that this installation process is similar to the automated process, but will install the jar and scripts in the ~/.clojure directory, rather than its typical location.
...
# Download and expand the installation tar file
curl -O https://download.clojure.org/install/brew/install-clj.tar.gz
tar xzf install-clj.tar.gz -C $HOME/.clojure

# Replace PREFIX in install-clj to reference the jar
perl -pi.bak -e "s,PREFIX,$HOME/.clojure,g" $HOME/.clojure/install-clj

# Add the scripts to the path
chmod +x $HOME/.clojure/install-clj
chmod +x $HOME/.clojure/clj
export PATH=$PATH:$HOME/.clojure
----
